...
impl BabyStepGiantStep {
    /// Build a BSGS table for $x \in [-\mathrm{max\_abs}, \mathrm{max\_abs}]$. Uses only
    /// group additions — no scalar multiplications.
    pub fn new(max_abs: u64) -> Self {
        // Cover 2·max_abs + 1 values. Balance baby/giant sizes: m ≈ √(2·max_abs+1), odd
        // so the baby table is symmetric around 0.
        let range_size = 2u64.saturating_mul(max_abs).saturating_add(1);
        let mut m = (range_size as f64).sqrt().ceil() as u64;
        if m < 1 { m = 1; }
        if m % 2 == 0 { m += 1; }
        let k = (m - 1) / 2;
...
impl CheatingDealerDlogSolver {
    pub fn new(n: usize, m: usize) -> Self {
        ...
        // BSGS covers [-(Z-1), Z-1] in one pass.
